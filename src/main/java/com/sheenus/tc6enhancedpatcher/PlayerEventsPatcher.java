...
public class PlayerEventsPatcher implements IClassTransformer{
	
	@Override
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
		if(name.contains("thaumcraft.common.lib.events.PlayerEvents")) {
	...
	public byte[] patchClassInJar(String name, byte[] bytes, String ObfName, File location) {
		...
		try {
			...
			if (entry == null) {
				PatcherLoader.log.info(entry + " not found in " + location.getName());
			} else { 
